...
class Vector2:
    ...
    Represents an n-dimensional numeric vector. ``Vector2`` objects support
    ...
    def __init__(self, values, keep: bool = False):
        if keep is not False:
            warnings.warn(
                "the 'keep' argument has been deprecated",
                DeprecationWarning,
            )
        if isinstance(values, Vector2):
            self.data = values.data
        else:
            self.data = array.array("d", values)

    def __repr__(self):
        return f"{self.__class__.__name__}(({', '.join(repr(v) for v in self.data)}))"

    def _vectorOp(self, other, op):
        if isinstance(other, Vector2):
            assert len(self) == len(other)
            return self.__class__(op(a, b) for a, b in zip(self.data, other.data))
        if isinstance(other, Number):
            return self.__class__(op(v, other) for v in self.data)
    ...
    def _unaryOp(self, op):
        return self.__class__(op(v) for v in self.data)
    ...
    def __eq__(self, other):
        if isinstance(other, (list, tuple)):
            # bw compat Vector2([1, 2, 3]) == [1, 2, 3]
            other = array.array("d", other)
